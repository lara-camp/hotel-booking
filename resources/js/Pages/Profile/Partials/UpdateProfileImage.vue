<template>
  <form class="relative md:w-1/2 h-auto max-h-[18rem] bg-slate-200/50 rounded">
    ...
    <img :src="previewProfilePhoto || user.profile_img" :alt="`Profile image of ${user.name}`"
      v-if="previewProfilePhoto || user.profile_img" class="object-cover w-full h-full rounded">
    ...
    <Button icon="pi pi-save" class="!absolute bottom-0 left-16 bg-indigo-700 hover:bg-indigo-700" :loading="form.processing" @click="submit"/>
  </form>
</template>
